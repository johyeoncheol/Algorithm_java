...
public class Expert_6809_Bitmask {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			perm(0, 0, 0, 0);// 라운드번호(횟수), 규영이점수, 인영이 점수
	...
	public static void perm(int round,int flag, int sum1, int sum2) {
		if(round == N) {//모든 라운드 끝.총점 비교
			if(sum1 > sum2) win++;
			if(sum1 < sum2) lose++;
			return;
		}
		//한 라운드 당 9개의 숫자를 가지고 비교할 건데, 이전 라운드에서 선택이 안된 숫자를 선택
		for (int i = 0; i < N; i++) {
			if((flag & 1<<i) == 0) { //1번째 비트가 사용 안된 것
				if(num1[round] > num2[i]) {//규영이의 카드 > 인영이의 카드 
					perm(round+1,flag | 1 << i,sum1+num1[round]+num2[i],sum2);
				}else {
					perm(round+1,flag | 1 << i,sum1,sum2+num1[round]+num2[i]);
				}
			}
		}
